...
void display_cfg_apply_event_cb(lv_event_t* e) // defocused the textarea
{
	...
	bool dis = lv_obj_has_state(ui_sleep_mode_cfg, LV_STATE_CHECKED); // true always on

	struct view_data_display cfg;
	memset(&cfg, 0, sizeof(cfg));
	if(target == ui_sleep_mode_cfg)
	{
		if(!dis)
		{
	...
	else if(target == ui_sleep_mode_time_cfg)
	{ // set a time, now it must be
	  // not always on
		cfg.sleep_mode_en = true;
	}
	char* p_time = lv_textarea_get_text(ui_sleep_mode_time_cfg);
	if(dis || !p_time)
	{
		cfg.sleep_mode_time_min = 0;
	}
...
void brighness_update_callback(lv_event_t* e) {
	...
	{
		lv_obj_clear_state(ui_sleep_mode_cfg, LV_STATE_CHECKED);
		// lv_obj_clear_flag(ui_PnlSleepModeTime, LV_OBJ_FLAG_HIDDEN);
	}
	else
	{
		lv_obj_add_state(ui_sleep_mode_cfg, LV_STATE_CHECKED);
	}
	char str[16] = {0};
	sprintf(str, "%d", cfg.sleep_mode_time_min);
	lv_textarea_set_text(ui_sleep_mode_time_cfg, str);
}
